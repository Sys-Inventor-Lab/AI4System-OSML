...
sys.path.append("../")
from configs import A_FEATURES, A_LABELS, ROOT, MAX_VAL, MIN_VAL
from utils import *
...
pretrained_model_dir = ROOT + "/models/pretrained/Model_A/"
pretrained_model_path = pretrained_model_dir + "Model_A.ckpt"
...
class Model_A:
    def __init__(self, tl=False, epoch=10, BATCH=256, learning_rate=0.001, drop_rate=0.3, is_train=False):
        ...
        self.data_valid = []
        self.max_min = {}
        self.max_min["input_max"] = [MAX_VAL[key] for key in A_FEATURES]
        self.max_min["input_min"] = [MIN_VAL[key] for key in A_FEATURES]
        ...
        if tl:
            ...
            if checkpoint and tf.train.checkpoint_exists(checkpoint.model_checkpoint_path):
                self.saver.restore(self.sess, pretrained_model_path)
                print_color("Pretrained Model A load successfully.", "green")
            else:
                print_color("Pretrained model is not found in {}.".format(pretrained_model_dir))
        else:
...
def train_model(tl):
    model = Model_A(tl=tl, is_train=True)
...
def test_model():
    ...
    output=model.sess.run(model.error, feed_dict={model.state: state_test, model.label: label_test})

...
pretrained_model_dir = ROOT + "/models/pretrained/Model_B/"
pretrained_model_path = pretrained_model_dir + "Model_B.ckpt"
    ...
    def __init__(self, tl=False, epoch=10, BATCH=256, learning_rate=0.001, drop_rate=0.3, is_train=False):
        ...
        if tl:
            ...
            if checkpoint and tf.train.checkpoint_exists(checkpoint.model_checkpoint_path):
                ...
                print_color("Pretrained Model B load successfully.", "green")
            else:
...
def train_model(tl):
    model = Model_B(tl=tl, is_train=True)

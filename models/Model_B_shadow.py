...
# Configurations
model_dir = ROOT + "/models/Model_B_shadow/"
model_path = model_dir + "Model_B_shadow.ckpt"
log_dir = ROOT + "/models/logs/"
log_path = log_dir + "Model_B_shadow_loss.csv"
data_dir = ROOT + "/data/data_process/Model_B_shadow/"
data_path_train = data_dir + "Model_B_shadow_train.csv"
data_path_test = data_dir + "Model_B_shadow_test.csv"
data_path_valid = data_dir + "Model_B_shadow_valid.csv"
max_min_path = ROOT + "/data/data_process/max_min/max_min_Model_B_shadow.txt"


class Model_B_shadow:
    def __init__(self, epoch=10, BATCH=256, learning_rate=0.0001):
        self.learning_rate = learning_rate
        self.epoch = epoch
        self.BATCH = BATCH
        self.input_size = len(B_SHADOW_FEATURES)
        self.output_size = len(B_SHADOW_LABELS)
        self.data_train = []
        self.data_valid = []
        with open(max_min_path, "r") as f:
            ...
            self.max_min["output_max"] = max_min["max"][self.input_size:]
            self.max_min["output_min"] = max_min["min"][self.input_size:]
    ...
    def off_normlize_for_output(self, output_arr):
        for arr in output_arr:
            for i in range(len(arr)):
                arr[i] = (arr[i] * (self.max_min["output_max"][i] - self.max_min["output_min"][i])) + \
...
def train_model():
    ...
    if os.path.exists(pkl_path_train) and os.path.getsize(pkl_path_train)>1:
        with open(pkl_path_train,"rb") as f:
            data_train=pickle.load(f)
    else:
        data_train = pd.read_csv(data_path_train)

    label_train = data_train.loc[:,B_SHADOW_LABELS].values
    state_train = data_train.loc[:,B_SHADOW_FEATURES].values
    #data_valid=pd.read_csv(data_path_valid)
    #state_valid = data_valid.loc[:, B_SHADOW_FEATURES].values
    #label_valid = data_valid.loc[:, B_SHADOW_LABELS].values
    #model_b_shadow.handleData(state_train, label_train, state_valid, label_valid)
    model_b_shadow.handleData(state_train, label_train)
    model_b_shadow.trainNetwork()


def test_model():
    model_b_shadow = Model_B_shadow()
    pkl_path_test = data_path_test.replace(".csv",".pkl")
    if os.path.exists(pkl_path_test) and os.path.getsize(pkl_path_test) > 1:
        with open(pkl_path_test,"rb") as f:
            data_test=pickle.load(f)
    else:
        data_test = pd.read_csv(data_path_test)
    state_test = data_test.loc[:, B_SHADOW_FEATURES].values
    label_test = data_test.loc[:, B_SHADOW_LABELS].values
    output=model_b_shadow.sess.run(model_b_shadow.error,feed_dict={model_b_shadow.state: state_test,model_b_shadow.label: label_test})
    error = pd.DataFrame(data=output,columns=B_SHADOW_LABELS)
    print(error.mean())

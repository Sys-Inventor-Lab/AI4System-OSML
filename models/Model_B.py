...
sys.path.append("../")
from configs import B_FEATURES, B_LABELS, ROOT
from utils import *
...
max_min_path = ROOT + "/data/data_process/max_min/max_min_Model_B.txt"
...
class Model_B:
    ...
    def use_model(self, input_arr):
        """
        Use Model-B to infer deprication policies.
        :param input_arr: Model-B features
        :return: Model-B labels
        """
    ...
    def create_network(self):
        ...
        self.loss = tf.reduce_mean(tf.square(tf.multiply(
            tf.divide(self.label, self.label + tf.constant([0.1 ** 10] * self.output_size, dtype=tf.float32)),
            self.label - self.label_conv)))
...
def train_model():
    ...
    #state_valid = data_valid.loc[:, B_FEATURES].values
    #label_valid = data_valid.loc[:, B_LABELS].values

    model_b.handleData(state_train, label_train)
    model_b.trainNetwork()
...
def test_model():
    ...
    output=model_b.sess.run(model_b.error, feed_dict={model_b.state: state_test, model_b.label: label_test})
